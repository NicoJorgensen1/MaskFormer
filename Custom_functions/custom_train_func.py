...
def launch_custom_training(FLAGS, config, dataset, epoch=0, run_mode="train", hyperparameter_opt=False):
    config.SOLVER.MAX_ITER = FLAGS.epoch_iter * (1 if all(["train" in run_mode, epoch>0, "vitrolife" in dataset]) else 1)  # Increase training iteration count for precise BN computations
    if all(["train" in run_mode, "vitrolife" in dataset, hyperparameter_opt==True]):                        # If we are optimizing hyperparameters on the vitrolife dataset...
        config.SOLVER.MAX_ITER = int(FLAGS.epoch_iter * 1)                                                  # ... the epochs will contain only ~5000 samples, i.e. approximately five epochs...
