...
def launch_custom_training(FLAGS, config, dataset, epoch=0, run_mode="train", hyperparameter_opt=False):
    ...
    if all(["train" in run_mode, hyperparameter_opt==True]):
        if "vitrolife" in dataset: config.SOLVER.MAX_ITER = int(FLAGS.epoch_iter * (3 if FLAGS.use_per_pixel_baseline else 6))      # ... Transformer and ResNet backbones need a little more data to do well while searching...
        elif "ade20k" in dataset: config.SOLVER.MAX_ITER = int(FLAGS.epoch_iter * (2 if FLAGS.use_per_pixel_baseline else 5)/20)    # ... the epochs will contain only ~1000 samples, i.e. 1/20 of the total samples...
