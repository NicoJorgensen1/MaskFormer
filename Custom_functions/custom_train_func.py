...
def launch_custom_training(FLAGS, config, dataset, epoch=0, run_mode="train", hyperparameter_opt=False):
    ...
    if all(["train" in run_mode, hyperparameter_opt==True]):
        ...
        elif "ade20k" in dataset: config.SOLVER.MAX_ITER = int(FLAGS.epoch_iter * (1 if FLAGS.use_per_pixel_baseline else 2)/100)   # ... little more data to do well while searching...
