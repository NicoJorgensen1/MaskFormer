...
def launch_custom_training(FLAGS, config, dataset, epoch=0, run_mode="train", hyperparameter_opt=False):
    ...
    if all(["train" in run_mode, hyperparameter_opt==True]):
        if "vitrolife" in dataset: config.SOLVER.MAX_ITER = int(FLAGS.epoch_iter * (2 if FLAGS.use_per_pixel_baseline else 4))  # ... Transformer and ResNet backbones need a ...
        elif "ade20k" in dataset: config.SOLVER.MAX_ITER = int(FLAGS.epoch_iter * (2 if FLAGS.use_per_pixel_baseline else 4)/20)# ... little more data to do well while searching...
