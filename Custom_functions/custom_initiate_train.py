...
import os
from pickletools import string1                                                                                           # Used to navigate the folder structure in the current os
...
# Get the FLAGS input arguments, the initial config and the handle for the log file
FLAGS, cfg, log_file = setup_func()                                                                 # Get the FLAGS and config variables
...
#     if FLAGS.HPO_current_trial < FLAGS.num_trials:
...
#     other_study = optuna.create_study(direction="maximize")
...
if FLAGS.hp_optim:
    FLAGS, cfg, trial =  perform_HPO(FLAGS, cfg, log_file)
