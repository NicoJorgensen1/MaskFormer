...
def putModelWeights(config, delete_remaining=False):
    model_files = [x for x in os.listdir(config.OUTPUT_DIR) if all([y in x for y in ["model", "_epoch_", ".pth"]].extend(~np.isnan(extractNumbersFromString(x))))]  # Find all saved model checkpoints
